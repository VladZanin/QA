...
average_score=(opts[:first].to_f+opts[:second].to_f+opts[:third].to_f+opts[:fourth].to_f+opts[:fifth].to_f+opts[:sixth].to_f)/6



puts "The summury of the following numbers (#{opts[:first]}, #{opts[:second]}, #{opts[:third]}, #{opts[:fourth]}, #{opts[:fifth]}, #{opts[:sixth]},) is #{average_score}"